...
--changeset thibault:add_projects

ALTER TABLE `courses` ADD `type` ENUM('course','project') ;

UPDATE `courses` SET type = 'course' WHERE 1;

CREATE TABLE `courses_basic` (
  `id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

ALTER TABLE `courses_basic`
  ADD PRIMARY KEY (`id`);

ALTER TABLE `courses_basic`
  ADD CONSTRAINT `courses_basic_ibfk_1` FOREIGN KEY (`id`) REFERENCES `courses` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

INSERT INTO courses_basic (id) SELECT id FROM courses;
...
ALTER TABLE `courses_project`
  ADD PRIMARY KEY (`id`);

ALTER TABLE `courses_project`
  ADD CONSTRAINT `courses_project_ibfk_1` FOREIGN KEY (`id`) REFERENCES `courses` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
CREATE TABLE `links_projects_criterias` (
  `project_id` int(11) NOT NULL,
  `criteria_id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

ALTER TABLE `links_projects_criterias`
  ADD UNIQUE KEY `project_id` (`project_id`,`criteria_id`),
  ADD KEY `criteria_id` (`criteria_id`);

ALTER TABLE `links_projects_criterias`
  ADD CONSTRAINT `links_projects_criterias_ibfk_1` FOREIGN KEY (`project_id`) REFERENCES `courses_project` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `links_projects_criterias_ibfk_2` FOREIGN KEY (`criteria_id`) REFERENCES `criterias` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `submissions`
  ADD CONSTRAINT `submissions_ibfk_1` FOREIGN KEY (`session_id`) REFERENCES `teach_sessions` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `submissions_ibfk_2` FOREIGN KEY (`project_id`) REFERENCES `courses_project` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
