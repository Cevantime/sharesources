...
//    }
	...
	private function deploy($args) {
		Core_utils::line("The script will first install a new database for the "
				. "app on your system. It requires a functionnal mysql server.");
		
		$this->installdb($args);
		
		Core_utils::line('Updating composer....');
		passthru('php tools/composer self-update');
		
		Core_utils::line('Running composer...');
		passthru('php tools/composer update --working-dir=application/');
		
		Core_utils::line('The script will now run npm in order to install the '
				. 'frontend libraries.');
		...
		if( ! Core_utils::is_os('windows')){
			...
			Core_utils::line('It requires you enter your root password.');
			$cmd = 'sudo '.$cmd;
